...
def compare_strings(doc, a, win, step):
    ...
    temp = np.zeros((q1.shape[0], 1))
    for i in range(0, q1.shape[1] - win, step):
        temp = np.hstack((temp, np.quantile(q1[:, i: i + win], a, axis = 1)))
        
    tfidf_1 = tfidf_vectorizer.transform(doc)
    dense = tfidf_1.toarray()
    dense[dense == 0] = np.nan
    q = np.nanquantile(dense, a, axis = 1)    
    return np.hstack((temp[:, 1:], q.T, numbers_ar))
...
def compare_strings_one(string, a, win, step):
    ...
    return np.hstack((temp[:, 1:].reshape(1, -1), q.reshape(1, -1), numbers_ar.reshape(1, -1)))
...
data = list(building['short_address']) + list(building['full_address'])
# Токенизация данных
...
win, step = 10, 5
emb_adr = compare_strings(new_df['address'], a, win, step)
emb_tar = compare_strings(new_df['full_address'], a, win, step)
...
params = [
    {'loss_function': 'Logloss', 'boosting_type': 'Plain', 'bootstrap_type': 'Bernoulli', 'depth': 4, 'colsample_bylevel': 0.14344097640056408, 'l2_leaf_reg': 1, 'iterations': 500, 'subsample': 0.9314298834061376},
    # {'loss_function': 'Logloss', 'boosting_type': 'Plain', 'bootstrap_type': 'Bernoulli', 'depth': 4, 'colsample_bylevel': 0.1180814008939229, 'l2_leaf_reg': 78, 'iterations': 1489, 'subsample': 0.940998383429287},
    ]
